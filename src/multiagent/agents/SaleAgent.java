...
/**
* SaleAgent类用于对汽车制造供应链上供应商行为的的模拟。
...
public class SaleAgent extends Agent{
...
//					声明ServiceEntity对象list
					MyList<ServiceEntity> saleservicelist=new MyList<ServiceEntity>();
					try {
						//获取服务,Supply，Manul,sell
						saleservicelist=getSaleService(agentEntity[0].getAgentName());
...
//						添加消息内容
						msg.setContentObject(saleservicelist);
//						发送消息
						send(msg);
						//存储交互信息，
//						获取通信时间
...
//							消息ID是注册服务消息
						case "Cloud-result":
							if (checkAction()) {
								System.out.println("Sale:"+"Agent "+getLocalName()+": Agree");
								...
								System.out.println("Sale:"+"Agent "+getLocalName()+": Refuse");
...
//	获取公司的服务列表，返回一个列表参数
	private static MyList<ServiceEntity> getSaleService(String str)throws Exception {
